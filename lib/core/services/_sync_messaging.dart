  ...
  Future<void> fetchMessages(String sessionId) async {
    ...
    try {
      ...
      try {
        ...
        final cachedCursorSeq = _maxCachedMessageSeq(
        ...
        // Which window to ask for is a pure decision — see
        // MessageCursorManager.computeFetchWindow. Everything below this call is
        // the effect: logging, and recording where a first load started.
        final window = _cursorManager.computeFetchWindow(
          sessionId,
          isFirstLoad: isFirstLoad,
          forceTailRefresh: forceTailRefresh,
        ...
        } else if (cursorSeq != 0 && forceTailRefresh) {
          logger.debug(
        ...
        if (Sync._isTransientConnectionError(error)) {
          ...
          unawaited(fetchSpan.finish());
          logger.info(
            '[fetchMessages] $sessionId transient fetch error: $error',
          );
          _notifySessionMessagesChanged(sessionId);
          _notifyDataChanged({SyncDomain.messages, SyncDomain.sessions});
          return;
