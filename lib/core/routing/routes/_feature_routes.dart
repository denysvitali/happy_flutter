...
/// Remaining feature routes: machines, artifacts, terminal, voice, and the
/// changelog.
///
...
List<RouteBase> get featureRoutes => [
  ...
  GoRoute(
    path: '/machines/:machineId/commands',
    name: 'machine-command',
    pageBuilder: (context, state) {
      final machineId = _pathParameter(state, 'machineId');
      if (machineId == null) {
        return _missingPathParameterPage(state, 'machineId');
      }
      return _slidePage(
        AuthGate(
  ...
  GoRoute(
    ...
    name: 'voice-language',
    pageBuilder: (context, state) =>
        _slidePage(const AuthGate(child: VoiceLanguageSettingsScreen()), state),
  ),
  GoRoute(
  ...
  GoRoute(
    ...
    pageBuilder: (context, state) =>
        _slidePage(const AuthGate(child: OfflineSttModelsScreen()), state),
  ),
  GoRoute(
    path: '/changelog',
    name: 'changelog',
