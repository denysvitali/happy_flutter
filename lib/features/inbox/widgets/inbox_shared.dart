...
class _InboxAdaptivePadding extends StatelessWidget {
  const _InboxAdaptivePadding({required this.child});
...
class _InboxEmptyList extends StatelessWidget {
  const _InboxEmptyList({required this.onRefresh, required this.children});
  ...
  Widget build(BuildContext context) {
    ...
    return Shimmer(
      child: ListView(
        ...
        children: [header(), row(), row(), row(), header(), row(), row()],
      ),
    );
  }
}
...
class InboxItemDescriptor {
  ...
  const InboxItemDescriptor.sectionHeader({required this.titleKey})
  ...
  const InboxItemDescriptor.incomingRequest({required this.incomingRequest})
    : type = InboxItemType.incomingRequest,
      titleKey = null,
      feedItem = null,
      userProfile = null;

  const InboxItemDescriptor.sentRequest({required UserProfile this.userProfile})
    : type = InboxItemType.sentRequest,
      titleKey = null,
      feedItem = null,
      incomingRequest = null;

  const InboxItemDescriptor.friend({required UserProfile this.userProfile})
