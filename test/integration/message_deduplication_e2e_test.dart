...
void main() {
  group('HTTP fetch deduplication', () {
    ...
    test('duplicate messages from server are merged by ID', () async {
      ...
      sync.testFetchMessagesOverride = (sid, afterSeq, limit) async {
        return _buildMessagesResponse([
          _makeEncryptedMessage('msg-1', seq: 5, content: 'updated content'),
        ]);
      };
      ...
      final withId = msgs!.where((m) => m['id'] == 'msg-1').toList();
      expect(
        withId.length,
        1,
    ...
    test(
      ...
      () async {
        ...
        sync.testFetchMessagesOverride = (sid, afterSeq, limit) async {
          return _buildMessagesResponse([
            _makeEncryptedMessage('msg-a', seq: 5, content: 'first'),
            _makeEncryptedMessage('msg-b', seq: 5, content: 'second'),
          ]);
        };
  ...
  group('socket inline deduplication', () {
    ...
    test('duplicate socket events for same message ID are merged', () async {
      ...
      final withId = msgs!.where((m) => m['id'] == 'msg-socket-1').toList();
      expect(
        withId.length,
        1,
    ...
    test('socket message with higher seq updates existing message', () async {
  ...
  group('optimistic message merge with server response', () {
    ...
    test(
      ...
      () async {
        ...
        final localCopies = msgs!.where((m) => m['id'] == 'local-1').toList();
        expect(
          localCopies,
          isEmpty,
          reason: 'Optimistic placeholder must be replaced by server record',
        );
    ...
    test('server ack replaces the exact optimistic placeholder by localId, '
        'not by repeated text or list position', () async {
  ...
  group('sidechain merge-path regression', () {
    ...
    test('sidechain messages must not remove Task tool-calls '
        'via shared localId', () {
      ...
      for (final taskId in ['task-1', 'task-2', 'task-3']) {
        final count = msgs!.where((m) => m['id'] == taskId).length;
        expect(
          count,
          1,
          reason:
  ...
  group('cross-source deduplication', () {
    ...
    test('HTTP fetch message + socket inline message for same ID '
        'results in one message', () async {
      ...
      sync.testFetchMessagesOverride = (sid, afterSeq, limit) async {
        return _buildMessagesResponse([
          _makeEncryptedMessage('msg-x', seq: 10, content: 'from http'),
        ]);
      };
      ...
      final withId = msgs!.where((m) => m['id'] == 'msg-x').toList();
      expect(
        withId.length,
        1,
        reason: 'Same ID from HTTP and socket must result in one entry',
      );
    });

    test('multiple rapid fetches do not create duplicates', () async {
      ...
      sync.testFetchMessagesOverride = (sid, afterSeq, limit) async {
        return _buildMessagesResponse([
          _makeEncryptedMessage('msg-1', seq: 3, content: 'message one'),
          _makeEncryptedMessage('msg-2', seq: 4, content: 'message two'),
          _makeEncryptedMessage('msg-3', seq: 5, content: 'message three'),
        ]);
      };
      ...
      for (final id in ['msg-1', 'msg-2', 'msg-3']) {
        final count = msgs!.where((m) => m['id'] == id).length;
        expect(
          count,
          1,
    ...
    test('REST-acked message followed by later socket echo keeps one '
        'logical message', () async {
    ...
    test('REST-acked message followed by later fetch overlap keeps one '
        'logical message', () async {
