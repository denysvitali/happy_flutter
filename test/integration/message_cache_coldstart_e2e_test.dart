...
void main() {
  ...
  group('cache restoration and display', () {
    ...
    test('cached messages are available immediately after '
    ...
    test('cached messages with correct seq set cursor properly', () async {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Cursor==server means no fetch should occur '
            '(already caught up)',
      );
    });

    test('empty cache results in first-load behavior', () async {
  ...
  group('server reconciliation with cache', () {
    ...
    test('server delta merges with cached messages', () async {
      ...
      sync.testFetchMessagesOverride = (sid, afterSeq, limit) async {
        expect(afterSeq, 5, reason: 'Delta fetch should start at cursor seq=5');
        return _buildResponse([
    ...
    test('server returns newer version of cached message', () async {
      ...
      sync.testFetchMessagesOverride = (sid, afterSeq, limit) async {
        return _buildResponse([
          _makeEncryptedMessage('msg-1', seq: 1, content: 'UpdatedContent'),
        ]);
      };
      ...
      final matching = result!.where((m) => m['id'] == 'msg-1').toList();
      expect(matching.length, 1, reason: 'Upsert should not duplicate msg-1');
    });

    test(
      'string encrypted server message is not skipped as unchanged',
      () async {
        const sessionId = 'sess-string-update-1';
        sync.testSessions[sessionId] = _makeSession(sessionId, lastSeq: 2);
        ...
        expect(matching.single['content'], 'Updated string content');
        expect(
          sync.sessionMessageCursors[sessionId],
          2,
          reason:
              'Processing the string-form encrypted update should advance '
              'the cursor past the fetched seq',
        );
      },
    );

    test('cache-only messages preserved when server has no new data', () async {
  ...
  group('multiple sessions cache interaction', () {
    ...
    test('cache for multiple sessions is independent', () {
      ...
      expect(
        idsA.intersection(idsB),
        isEmpty,
        reason: 'Sessions A and B should have disjoint message sets',
      );
    });

    test('fetching one session preserves other session cache', () async {
...
Map<String, dynamic> _makeEncryptedMessage(
  ...
  return {
    ...
    'content': stringContent
        ? base64Encode(output)
        : {'t': 'encrypted', 'c': base64Encode(output)},
    'createdAt': 1700000000000 + seq * 1000,
  };
}
