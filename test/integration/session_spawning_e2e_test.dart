...
void main() {
  group('createSession E2E flow', () {
    ...
    test('successful spawn creates optimistic placeholder when server '
      ...
      _stubAllSyncs(sync, sessionsFn: () async {
  ...
  group('_sessionSpawnedAt lifecycle', () {
    ...
    test('full fetch preserves optimistic sessions within 60s window',
        () async {
      // Spawn session
    ...
    test('_sessionSpawnedAt is cleaned up on 404 in fetchMessages',
        () async {
  ...
  group('_resolveSendTargetSession readiness checks', () {
    ...
    test('recently spawned session is considered ready — no auto-restore',
        () async {
    ...
    test('session with recent starting lifecycle is considered ready',
        () async {
    ...
    test('auto-restore creates placeholder if restored session not in map',
        () async {
    ...
    test('auto-restore falls back to original session on RPC failure',
        () async {
  ...
  group('sendMessage encryption recovery', () {
    ...
    test('3-step escalation: fetchSingle → invalidate → force full fetch',
        () async {
      ...
      expect(fetchSingleCalled, isTrue,
      ...
      expect(forceFetchCalled, isTrue,
          reason: 'Step 3: _forceFullFetchNext must be set');
    });

    test('throws if encryption still missing after all 3 recovery attempts',
        () async {
  ...
  group('sendMessage session lookup recovery', () {
    ...
    test('creates placeholder when session not found after all recovery',
        () async {
  ...
  group('waitForAgentReady', () {
    ...
    test('returns true immediately for running session with recent timestamp',
        () async {
      final now = DateTime.now().millisecondsSinceEpoch;
  ...
  group('sendMessage optimistic insert', () {
    ...
    test('optimistic message appears in session messages immediately',
        () async {
      ...
      final notifiedSessionId = await notified.future
  ...
  group('full spawn → sendMessage E2E', () {
    ...
    test('createSession → sendMessage → session comes online → message sent',
        () async {
  ...
  group('_primeSessionFromSpawnResult', () {
    ...
    test('skips placeholder if session already exists', () async {
      ...
      expect(sync.sessions['already-exists']!.metadataVersion,
          originalVersion);
    });
