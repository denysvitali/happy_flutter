...
void main() {
  ...
  // ──────────────────────────────────────────────────────────────────
  test('no-progress counter resets when an orphan is resolved via the '
      'prompt fallback axis', () async {
    ...
    expect(
      ...
      0,
      reason:
          'orphan resolution via the prompt fallback axis '
  ...
  // ──────────────────────────────────────────────────────────────────
  test('suppression window is set to now+throttle when the walk-back '
      'exhausts with hasMoreOlder=true — subsequent sweeps inside the '
      'window are no-ops', () async {
    sync.testSetSessionMessages('s1', [
      <String, dynamic>{
        'id': 'orph-1',
        'isSidechain': true,
  ...
  // ──────────────────────────────────────────────────────────────────
  test('orphanCountForSession returns the visible orphan count for a '
  ...
  // ──────────────────────────────────────────────────────────────────
  test('persistent-orphan orphans are reported at info level on the '
      'first sweep and the orphan-count field stays stable across '
      'repeated sweeps with the same signature', () async {
    sync.testSetSessionMessages('s1', [
      <String, dynamic>{
        'id': 'orph-1',
        'isSidechain': true,
    ...
    expect(
      ...
      2,
      reason:
          'history-exhausted branch must preserve orphans '
    ...
    for (var i = 0; i < 5; i++) {
      ...
      expect(
        ...
        2,
        reason:
            'orphan count must remain stable across repeated '
    ...
    expect(
      ...
      0,
      reason:
          'history-exhausted branch must clear the no-progress '
