  ...
  group('ServicesApi', () {
    ...
    group('connectService', () {
      ...
      test('throws exception when service name is empty', () async {
        expect(
          ...
          throwsA(
            isA<ServicesApiException>().having(
              (e) => e.message,
              'message',
      ...
      test('throws exception on non-200 response', () async {
        ...
        expect(
          ...
          throwsA(
            isA<ServicesApiException>()
                .having((e) => e.statusCode, 'statusCode', 401)
                .having(
                  (e) => e.message,
                  'message',
                  contains('Failed to connect github'),
                ),
          ),
        );
      });
    ...
    group('isServiceConnected', () {
      test('returns true when service is connected (200)', () async {
        when(mockClient.get('/v1/connect/github')).thenAnswer(
          (_) async => Response(
            data: {'connected': true},
            statusCode: 200,
            requestOptions: RequestOptions(path: ''),
    ...
    group('getAllConnectionStatus', () {
      test('returns connection status for all services', () async {
        ...
        when(mockClient.get('/v1/connect/claude')).thenAnswer(
          (_) async => Response(
            data: {},
            statusCode: 200,
            requestOptions: RequestOptions(path: ''),
        ...
        when(mockClient.get('/v1/connect/openai')).thenAnswer(
          (_) async => Response(
            data: {},
            statusCode: 404,
            requestOptions: RequestOptions(path: ''),
        ...
        expect(result, {'github': true, 'claude': true, 'openai': false});
      });
      ...
      test('handles errors gracefully', () async {
        ...
        expect(result, {'github': false, 'claude': false, 'openai': false});
      });
    });
