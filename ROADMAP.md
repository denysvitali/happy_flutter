...
The latest event raised its count to four on build 273700 at 17:41 UTC, but
Loki shows that path auto-restored the session, accepted the send three seconds
later, and observed the response. Local daemon restarts exposed the root cause:
graceful shutdown wrote an explicit replacement-daemon handoff marker, then the
replacement treated it as ordinary stale state because startup respawn defaults
off. happy-cli-go 4f66380 now honors a recent exact handoff marker, prevents a
late host-PID CAS from reviving terminal state, fences canceled Kubernetes
reconcile results, and stops server message persistence from asserting runtime
liveness. Restorable lifecycle history is info-level in Flutter; blocked sends
remain warnings. Keep 8679 open until the daemon and app fixes have a clean
post-deploy window.
