...
| ANR (foreground `nativePollOnce` + background `__sfvwrite`) | Fatal | 8 | Open — thread dump arrived 2026-08-26, unsymbolicated | First ANRs ever captured 2026-06-09 (HAPPY_FLUTTER-3D6/3D7); bodies were DLQ'd then (see k2-gitops 5ca1e85 fix). The 2026-08-26 event (8 total, mechanism `AppExitInfo`, build 270300) finally carries the dump: the main thread sits under `nativePollOnce` with **512 consecutive `libapp.so` frames cycling through the same 4 addresses** — unbounded mutual recursion in Dart code on the UI thread, not looper idleness. The frames cannot be symbolicated on build 270300 (obfuscated AOT, no symbols uploaded). Follow-up 2026-08-27: the 15:46:54 recurrence on build 271100 (post-fa24c46a) is **still address-only** — Android's AppExitInfo tombstone resolves ELF symbols only, and Dart snapshot code has none regardless of obfuscation, so dropping obfuscation did not make these dumps readable; naming the cycle needs manual symbolication against the release `libapp.so`. That ANR fired 10 s after the todo_get TypeError (issue 8570) in the same launch and chat-open flow, matching the ErrorBoundary/ErrorWidget-recursion mechanism already fixed in 3eef2c38 — if follow-on ANRs persist on builds ≥ 3eef2c38, the recursion is independent and symbolication becomes the priority. Build mapping (2026-08-27 16:30 UTC): 271100 = 6b26999e (pre-fix), 271200 = 65008170 (pre-fix — the 3eef2c38 fix postdates it), **271300 = cbe1bf22 is the first build carrying the fix**. A third same-day recurrence (16:04:36, mechanism `AppExitInfo`, 521 frames, 504× `libapp.so`, same CPH2653 launch chain as the 15:46 pair) is also on 271100 — expected pre-fix noise, not evidence the fix failed. Re-classify the family only when an event lands on a build ≥ 271300. |
| TypeError `'List<dynamic>' is not a subtype of 'String?'` at chat open | Error | 3 | Fix on main, shipped automatically on the next `main` commit | Recurred 2026-08-27 15:46 on build 271100 (issues 8570/8571 + ANR 3529). Codex MCP `todo_get` results arrive as `{content:[{type:text,text}], status}` maps; `TaskGet` did `map['content'] as String?` during the post-frame todo push (`scheduler library`), then ErrorBoundary/ErrorWidget recursion ANR'd the UI isolate. Flatten MCP content blocks in `_resultText` and stop hard-casting wire fields that can be lists (`test/features/chat/tools/views/task_tool_view_test.dart`). Both 2026-08-27 events (18:08 build 270800 + 15:46 build 271100) predate the fix; **build 271300 is the first release carrying 3eef2c38** — verify zero recurrences on ≥ 271300 before closing. |
| `RpcException(handler_error, codex debug models: signal: killed)` | Error | 1 (2026-08-27 15:21, build 271100) | Open — daemon-side, watch | Issue 8603: `machineGetCodexModels` surfaced a daemon error whose `codex debug models` subprocess was SIGKILLed (OOM suspect on the daemon host); no matching `happy-daemon` Loki lines in the window. Client behavior is correct (error surfaced once, no crash). Re-classify only if it recurs. |
