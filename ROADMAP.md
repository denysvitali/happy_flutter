...
open defect awaits symbolication (TypeError at chat open, issue 8570) and
the first ANR thread dump arrived (unbounded `libapp.so` recursion,
unsymbolicated — obfuscation dropped in fa24c46a, so the next occurrence
on a newer build names the cycle). Everything else in the window is
...
| ANR (foreground `nativePollOnce` + background `__sfvwrite`) | Fatal | 8 | Open — thread dump arrived 2026-08-26, unsymbolicated | First ANRs ever captured 2026-06-09 (HAPPY_FLUTTER-3D6/3D7); bodies were DLQ'd then (see k2-gitops 5ca1e85 fix). The 2026-08-26 event (8 total, mechanism `AppExitInfo`, build 270300) finally carries the dump: the main thread sits under `nativePollOnce` with **512 consecutive `libapp.so` frames cycling through the same 4 addresses** — unbounded mutual recursion in Dart code on the UI thread, not looper idleness. The frames cannot be symbolicated on build 270300 (obfuscated AOT, no symbols uploaded), but Dart obfuscation was dropped 2026-08-26 (fa24c46a) — the next ANR on a build ≥ that commit names the recursive cycle natively. Nothing to do until then. |
| TypeError `'List<dynamic>' is not a subtype of 'String?'` at chat open | Error | 2 | Open — awaiting symbolicated recurrence | Build 270300, 2026-08-26 18:07–18:08, one device (issues 8570/8571). Tapping a session row pushes chat, and ~45 ms later ErrorBoundary catches the cast failure attributed to the scheduler library — inside the first chat-load microtask; the projection itself renders fine 3 ms later (107 rows, `_doInitialLoad completed elapsedMs=2`). Some wire field that is usually a string arrives as a list and hits an `as String?` cast during chat load. Frames are obfuscated-native with no uploaded symbols, so the cast site cannot be named from the event; Dart obfuscation dropped in fa24c46a, so the next occurrence on a build ≥ that commit carries a symbolicated stack — pin and harden the cast then. |
