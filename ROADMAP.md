...
Issue 8679 is the first 273500 event under that stable title: it reported a
pre-existing errored session at 16:00 UTC rather than accepting it as ready.
The event predates the standalone-daemon rollout at 16:02 UTC and confirms the
client guard; the daemon-owned lifecycle fix remains open.
...
daemon capability advertisement is the functional fix. GitOps commit d4502fa
now passes `HAPPY_KUBERNETES_GH_PROXY_URL` to the standalone daemon; verify the
repository-backed spawn. Argo reconciled that exact commit and reports the
application Synced and Healthy. The app emits one stable user-boundary warning
instead of duplicating the same RPC failure at both Sync and dialog layers,
while retaining machine/session/backend details in a breadcrumb.
...
Newest unresolved page (builds 271000–271500, CPH2653). One real
client defect on the current build, one reporting-level daemon error,
everything else stale or daemon-side.

**ErrorBoundary ANR loop, issue 3659, build 271500 (fatal).** Chat →
message-detail (CodexBash) → pop → home. First error is a null-check
in the widgets library 46 ms after the home pop; ErrorBoundary then
replaces the tree *above* MaterialApp. `_ErrorWidgetFallback` called
`Theme.of`, which threw, which built another ErrorWidget — 498
`libapp.so` frames cycling 4 addresses, same shape as the 271100
todo_get ANR but a different trigger. Fixed: ErrorWidget.builder
never looks up Theme/l10n, latches re-entry to a shrink box, and the
full-app takeover injects a minimal Material host. Regression in
`test/core/widgets/error_boundary_test.dart`.

**Codex SIGKILL, issues 8603/8606, builds 271100+271500.**
`machineGetCodexModels` logged `RpcException(handler_error, codex
debug models: signal: killed)` at error (Sentry). Daemon OOM on
`codex debug models`; catalog is optional chrome. Demoted to info
(`test/services/codex_model_catalog_cache_test.dart`).

Stale / not a Flutter defect this pass: todo_get TypeError 8570 last
seen on 271100 (fix in 3eef2c38, first ship 271300 — no recurrence on
271500); loop `method_unsupported`/`handler_offline` 8601/8602 on
271000 (fix already on main); session lifecycle 8607 on 271500
(daemon has no live process after 8 min background — client recovered
reload+socket); pipeline `no-encryption` 8604/8605 (socket beat DEK,
recovery ran, next seq notified ok); machineRPC SLOW/FAILED, spawn
unreachable, `/v2/sessions` receive-timeout — daemon/network.
...
process for the restored session. The chat-open TypeError (issue 8570) and follow-on ANR (3529) are
fixed on main: Codex MCP `todo_get` results used a `List` as `String?`.
...
Swept 50 unresolved issues (builds 269300–270300). Three client defects
fixed on main (see Production Bugs rows below): the `ServerConfigStorage`
pre-MMKV warning (258 events), the Kimi-429 usage warning spam (173), and
the `unmatched_optimistic` outbox false positive (issue 8566). One new
open defect (TypeError at chat open, issue 8570) is now pinned: Codex MCP
`todo_get` envelopes put a content-block list in `result.content`, and
`TaskGet` cast it as `String?` during the post-frame todo push. The ANR
thread dump (unbounded `libapp.so` recursion) was the ErrorWidget loop
that followed. Everything else in the window is
fallout from a **server brownout 16:29–18:09 UTC 2026-08-26** — clustered
receive timeouts on `/v2/sessions`, `/v1/machines`, `/v1/version`,
`/v1/account/settings` with RetryInterceptor budget exhaustion, spawn /
session-lifecycle / `handler_offline` failures, and 2 outbox dead-letters.
That shape matches the known Postgres-stall brownout family (see the
observability audits); treat those issues as server-side until the
server metrics for the window say otherwise.
...
| Session says running but daemon has no process | Warning (user-visible send failure) | 6 warnings / 5 sessions on 273300, 1 guarded open on 273500, plus the earlier 27-event burst | Client routing guard verified in 273500; daemon root cause open | Issues 8623/8626/8654/8674/8675 span multiple sessions. Same-window daemon logs show metadata-version CAS races, failed lifecycle marks, stale-session reconciliation, and zero live processes. Issue 8679 on 273500 used the new stable group and rejected a pre-existing errored session before the daemon rollout began. The client now makes `hasLifecycleError` terminal across readiness, recent-spawn, and send-resolution gates, restores before sending, and preserves the canonical `localId`. The durable fix remains a daemon-owned runtime epoch/lease and atomic process transition propagated through server state. |
...
| CI/CD | Done | 7-job pipeline (analyze, test + coverage, golden, build-debug, build-release, build-web, deploy-web), caching, automatic per-commit releases to GitHub (Dart obfuscation dropped 2026-08-26 — open-source app), Codecov |
