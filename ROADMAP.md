...
Argo rollout and a repository-backed spawn. The app emits one stable
user-boundary warning instead of duplicating the same RPC failure at both Sync
and dialog layers, while retaining machine/session/backend details in a
breadcrumb.
...
| Kubernetes backend advertised without `GHProxyURL` | Warning (session creation blocked) | 2 fingerprints / 1 spawn on 273300 | Client reporting fixed in 273500; GitOps fix d4502fa pushed | Issues 8672/8673 are the same explicit Kubernetes repository spawn. The `/workspace` path is valid, but the standalone daemon did not receive `HAPPY_KUBERNETES_GH_PROXY_URL`. k2-gitops now passes the in-cluster proxy URL; verify the Argo rollout and a repository-backed spawn. Flutter reports one stable dialog-boundary warning and keeps RPC context in a breadcrumb. |
